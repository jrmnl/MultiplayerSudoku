...
namespace MultiplayerSudoku.Logic
{
    ...
    public static class Sudoku
    {
        ...
        public static bool HasInputVariants(int?[,] board)
        {
            for (int rowIndex = 0; rowIndex < board.GetLength(0); rowIndex++)
            {
                for (int colIndex = 0; colIndex < board.GetLength(1); colIndex++)
                {
                    ...
                    if (element is null)
                    {
                        var reservedElements = board
                            .GetSquareElements(rowIndex, colIndex)
                            .Concat(board.GetRow(rowIndex))
                            .Concat(board.GetColumn(colIndex))
                            .Distinct()
                            .OfNotNullable();

                        var isAnyAllowed = AllowedSymbols.Except(reservedElements).Any();
                        if (isAnyAllowed) return true;
                    }
                }
            }
            return false;
